...
const softwareAreas = {
  tr: [
    { area: 'NLP / Yapay Zekâ Modeli', detail: 'Tıbbi terminoloji üzerinde özel eğitim' },
    { area: 'PDF & Word İşleme', detail: 'Orijinal belge düzenini koruyarak çıktı' },
    { area: 'Çok Dilli Destek', detail: '30+ dil, klinik terminoloji hassasiyeti' },
    { area: 'HBS API Entegrasyonu', detail: 'Mevcut hasta bilgi sistemine bağlantı' },
    { area: 'KVKK Uyumlu Altyapı', detail: 'Belgeler saklanmaz, şifreli aktarım' },
    { area: 'Bulut Tabanlı İşlem', detail: 'Kurulum gerektirmez, tarayıcıdan erişim' },
  ],
  en: [
    { area: 'NLP / AI Model', detail: 'Specially trained on medical terminology' },
    { area: 'PDF & Word Processing', detail: 'Output preserving original document layout' },
    { area: 'Multi-Language Support', detail: '30+ languages, clinical terminology accuracy' },
    { area: 'HIS API Integration', detail: 'Connection to existing hospital info system' },
    { area: 'GDPR-Compliant Infrastructure', detail: 'Documents not stored, encrypted transfer' },
    { area: 'Cloud-Based Processing', detail: 'No installation, browser access' },
  ],
}
  ...
  return (
    <motion.div {...fadeUp(index * 0.05)} className="border-b border-slate-200 last:border-0">
      <button onClick={() => setOpen((o) => !o)}
        ...
        <span className="text-base font-semibold text-dark group-hover:text-accent transition-colors duration-200 leading-snug">{faq.question}</span>
...
export default function TercumedPage() {
  const { lang } = useLang()
  const t = i18n[lang]
  const base = products.find((p) => p.slug === 'tercumed')
  const product = lang === 'en' && base.en ? { ...base, ...base.en } : base
  const areas = softwareAreas[lang]

  const scopeText = lang === 'en'
    ? 'Tercümed is an AI-powered medical document translation solution that removes language barriers in discharge summaries, consent forms, and clinical reports in seconds. Unlike general-purpose translation tools, its model is specifically trained on medical terminology — ensuring diagnoses, drug names, and procedure terms are translated with clinical accuracy. The three-step workflow (upload → select language → download) requires no training and can be used instantly by anyone from patient admission to clinical staff.'
    : 'Tercümed, taburculuk belgesi, onam formu ve klinik raporlardaki dil engelini saniyeler içinde kaldıran yapay zekâ destekli tıbbi belge çeviri çözümüdür. Genel amaçlı çeviri araçlarının aksine modeli tıbbi terminoloji üzerinde özel olarak eğitilmiştir; tanılar, ilaç adları ve prosedür terimleri klinik hassasiyetle çevrilir. Üç adımlık iş akışı (yükle → dil seç → indir) herhangi bir eğitim gerektirmez, hasta kabulünden klinisyenlere kadar herkes tarafından anında kullanılabilir.'
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-24 md:pt-28">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-end">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="text-base md:text-lg text-white/55 leading-relaxed max-w-lg mb-8">{product.tagline}</p>
              {base.website && (
                <a href={base.website} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/20 text-sm font-semibold text-white/70 hover:border-white/50 hover:text-white transition-all duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {areas.map((item, i) => (
              <motion.div key={i} {...fadeUp(i * 0.06)}
                className="flex items-start gap-4 p-6 rounded-2xl border border-slate-200 hover:border-accent/25 hover:bg-accent/2 transition-all duration-200 group">
                <div className="w-2 h-2 rounded-full bg-accent mt-1.5 shrink-0 group-hover:scale-125 transition-transform duration-200" />
                <div>
      ...
      <section className="py-16 md:py-24 border-t border-slate-100 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <motion.div {...fadeUp()} className="mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-dark leading-tight">
              {lang === 'en' ? 'Precision at every step' : 'Her adımda klinik hassasiyet'}
            </h2>
          </motion.div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <motion.div {...fadeUp(0.15)} className="rounded-2xl bg-accent p-6 flex flex-col justify-between min-h-35">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            ...
            <motion.div {...fadeUp(0.2)} className="rounded-2xl bg-accent/8 border border-accent/15 p-7 flex flex-col justify-between">
              <p className="text-xs font-bold tracking-widest uppercase text-accent">{product.stats?.[2]?.label}</p>
              <p className="text-5xl font-black text-accent leading-none mt-auto pt-6">{product.stats?.[2]?.value}</p>
            </motion.div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <motion.div {...fadeUp()} className="mb-20">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-dark leading-tight max-w-lg">
              {lang === 'en' ? 'Built for clinical environments' : 'Klinik ortamlar için tasarlandı'}
            </h2>
          </motion.div>
      ...
      {product.faqs && (
        <section className="py-24 md:py-32 border-t border-slate-100">
          <div className="max-w-7xl mx-auto px-6 lg:px-12">
            <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-12 lg:gap-20">
              <motion.div {...fadeUp()}>
                <p className="text-xs font-bold tracking-[0.2em] uppercase text-accent mb-3">SSS</p>
                <h2 className="text-3xl md:text-4xl font-bold text-dark leading-tight">
      ...
      <section className="py-24 md:py-28 border-t border-slate-100 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {products.filter((p) => p.slug !== 'tercumed').slice(0, 3).map((rel, i) => {
