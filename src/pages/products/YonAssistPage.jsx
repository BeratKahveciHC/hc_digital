...
const softwareAreas = {
  tr: [
    { area: 'NLP & Ses Tanıma', detail: 'Türkçe/İngilizce doğal dil işleme' },
    { area: 'Toplantı Transkripsiyon', detail: 'Otomatik özet ve eylem maddesi çıkarımı' },
    { area: 'Görev Yönetim Katmanı', detail: 'Atama, takip ve hatırlatma otomasyonu' },
    { area: 'Takvim & E-posta Entegrasyonu', detail: 'Teams, Zoom, Google Meet desteği' },
    { area: '7/24 Çalışan Altyapı', detail: 'Yüksek erişilebilirlik, kesintisiz servis' },
    { area: 'KVKK Uyumlu Veri Saklama', detail: 'Türkiye lokasyonlu, şifreli sunucular' },
  ],
  en: [
    { area: 'NLP & Voice Recognition', detail: 'Turkish/English natural language processing' },
    { area: 'Meeting Transcription', detail: 'Automatic summary and action item extraction' },
    { area: 'Task Management Layer', detail: 'Assignment, tracking, and reminder automation' },
    { area: 'Calendar & Email Integration', detail: 'Teams, Zoom, Google Meet support' },
    { area: '24/7 Infrastructure', detail: 'High availability, uninterrupted service' },
    { area: 'GDPR-Compliant Storage', detail: 'Turkey-based, encrypted servers' },
  ],
}
...
export default function YonAssistPage() {
  const { lang } = useLang()
  const t = i18n[lang]
  const base = products.find((p) => p.slug === 'yon-assist')
  const product = lang === 'en' && base.en ? { ...base, ...base.en } : base
  const areas = softwareAreas[lang]

  const scopeText = lang === 'en'
    ? 'Yön Assist is an AI-powered digital assistant that understands voice and written commands through NLP infrastructure, automatically extracts meeting summaries, manages tasks, and optimizes time planning. Integrated with your existing tool ecosystem (Teams, Zoom, calendar, email), it assigns meeting action items to relevant team members and tracks follow-up without manual effort. Operating 24/7 with enterprise-grade security, it serves organizations of all sizes across healthcare, finance, and corporate sectors.'
    : 'Yön Assist, NLP altyapısı aracılığıyla sesli ve yazılı komutları anlayan, toplantı özetlerini otomatik çıkaran, görevleri yöneten ve zaman planlamasını optimize eden yapay zekâ destekli bir dijital asistandir. Mevcut araç ekosistemine (Teams, Zoom, takvim, e-posta) entegre çalışarak toplantı eylem maddelerini ilgili kişilere atlar ve takip sürecini manuel çaba gerektirmeden başlatır. 7/24 kurumsal güvenlikle çalışan platform; sağlık, finans ve kurumsal sektörlerdeki her ölçekte organizasyona hizmet eder.'
      ...
      <section className="py-16 md:py-24 border-t border-slate-100 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <motion.div {...fadeUp()} className="mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-dark leading-tight">
              {lang === 'en' ? 'Your smartest team member' : 'En akıllı ekip üyeniz'}
            </h2>
          </motion.div>
            ...
            <motion.div {...fadeUp(0.1)} className="rounded-2xl bg-dark p-8 flex flex-col justify-between min-h-55">
              <div>
                <div className="w-8 h-8 rounded-lg bg-accent/10 flex items-center justify-center mb-4">
                  <span className="text-accent text-xs font-black">01</span>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <motion.div {...fadeUp()} className="mb-20">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-dark leading-tight max-w-lg">
              {lang === 'en' ? 'AI that works while you focus' : 'Siz odaklanırken yapay zekâ çalışır'}
            </h2>
          </motion.div>
      ...
      <section className="py-24 md:py-28 border-t border-slate-100 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {products.filter((p) => p.slug !== 'yon-assist').slice(0, 3).map((rel, i) => {
