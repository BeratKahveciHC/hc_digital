...
const softwareAreas = {
  tr: [
    { area: 'Otomatik Muhasebe Kaydı', detail: 'Fatura ve gider takibini otomatik olarak işler' },
    { area: 'ERP / HIS Entegrasyonu', detail: 'Mevcut hastane ve kurumsal sistemlere doğrudan bağlantı' },
    { area: 'Yapay Zekâ Tabanlı Tahmin', detail: 'Geçmiş verilere göre bütçe öngörüsü ve anomali tespiti' },
    { area: 'Birim Bazlı Raporlama', detail: 'Klinik ve departman düzeyinde detaylı maliyet analizi' },
    { area: 'Rol Tabanlı Erişim', detail: 'Kullanıcı yetkisine göre özelleştirilmiş yönetim paneli' },
    { area: 'Güvenli Finansal Altyapı', detail: 'BDDK uyumlu şifreleme ve katmanlı veri güvenliği' },
  ],
  en: [
    { area: 'Automated Accounting', detail: 'Processes invoices and expense tracking automatically' },
    { area: 'ERP / HIS Integration', detail: 'Direct connection to existing hospital and enterprise systems' },
    { area: 'AI-Based Forecasting', detail: 'Budget prediction and anomaly detection from historical data' },
    { area: 'Unit-Based Reporting', detail: 'Detailed cost analysis at clinic and department level' },
    { area: 'Role-Based Access', detail: 'Customized management dashboard by user authorization' },
    { area: 'Secure Financial Infrastructure', detail: 'BDDK-compliant encryption and layered data security' },
  ],
}
  ...
  return (
    <motion.div {...fadeUp(index * 0.05)} className="border-b border-slate-200 last:border-0">
      <button onClick={() => setOpen((o) => !o)}
        ...
        <span className="text-base font-semibold text-dark group-hover:text-primary transition-colors duration-200 leading-snug">{faq.question}</span>
...
export default function ButceYonetimPage() {
  const { lang } = useLang()
  const t = i18n[lang]
  const base = products.find((p) => p.slug === 'butce-yonetim')
  const product = lang === 'en' && base.en ? { ...base, ...base.en } : base
  const areas = softwareAreas[lang]

  const scopeText = lang === 'en'
    ? 'The Budget Management System is an AI-powered solution that enables healthcare and public institutions to manage their financial processes from a centralized platform. Integrated with ERP and HIS systems, it automates invoice processing, unit-based cost tracking, and budget forecasting. Managers gain financial transparency at every level — from clinical departments to administrative units — through real-time reports, reducing accounting workload and accelerating decision-making.'
    : 'Bütçe Yönetim Sistemi, sağlık ve kamu kurumlarının finansal süreçlerini merkezi bir platformdan yönetmesini sağlayan yapay zekâ destekli bir çözümdür. ERP ve HIS sistemleriyle entegre çalışarak fatura işleme, birim bazlı maliyet takibi ve bütçe öngörüsü görevlerini otomatize eder. Yöneticiler, gerçek zamanlı raporlarla klinik departmanlardan idari birimlere kadar her düzeyde finansal şeffaflık elde eder; muhasebe yükü azalır, karar alma süreçleri hızlanır.'
      ...
      <section className="py-16 md:py-24 border-t border-slate-100 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <motion.div {...fadeUp()} className="mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-dark leading-tight">
              {lang === 'en' ? 'Financial visibility at every level' : 'Her düzeyde finansal görünürlük'}
            </h2>
          </motion.div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <motion.div {...fadeUp(0.15)} className="rounded-2xl bg-primary p-6 flex flex-col justify-between min-h-35">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            ...
            <motion.div {...fadeUp(0.2)} className="rounded-2xl bg-primary/8 border border-primary/15 p-7 flex flex-col justify-between">
              <p className="text-xs font-bold tracking-widest uppercase text-primary">{product.stats?.[2]?.label}</p>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <motion.div {...fadeUp()} className="mb-20">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-dark leading-tight max-w-lg">
              {lang === 'en' ? 'Integrated financial control' : 'Entegre finansal kontrol'}
            </h2>
          </motion.div>
      ...
      <section className="py-24 md:py-28 border-t border-slate-100 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {products.filter((p) => p.slug !== 'butce-yonetim').slice(0, 3).map((rel, i) => {
